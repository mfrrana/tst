...
class Welcome extends CI_Controller {
    ...
    public function admission_form() {
        ...
        //$id=$data['auto_generated_id'];
        //$data['auto_id'] =  $data['auto_generated_id'];
    ...
    public function student_subject_form() {
        ...
        $data = array();
        $data['maincontent'] = $this->load->view('form/student_subject_form', $results, TRUE);
        $this->load->view('master', $data);
    }
    ...
    public function enrolement_form() {
        $data = array();
        $data['all_section'] = $this->stu_model->all_section_info();
        $data['all_level'] = $this->stu_model->all_level_info();
        $data['all_batch'] = $this->stu_model->all_batch_info();
        $data['all_class'] = $this->stu_model->all_class_info();
        $data['maincontent'] = $this->load->view('form/enrolement_form', $data, TRUE);
        $this->load->view('master', $data);
    }
    ...
    public function teacher_info() {
        ...
        $data['teacher_email2'] = $this->input->post('teacher_email2');

        $data['teacher_mobile'] = $this->input->post('teacher_mobile');
        $data['teacher_mobile2'] = $this->input->post('teacher_mobile2');

        $data['teacher_phone'] = $this->input->post('teacher_phone');
        $data['teacher_phone2'] = $this->input->post('teacher_phone2');
        ...
        if ($data['teacher_name'] == NULL || $data['teacher_address'] == NULL || $data['teacher_email'] == NULL ||
                $data['teacher_mobile'] == NULL || $data['teacher_phone'] == NULL || $data['teacher_designation'] == NULL ||
    ...
    public function save_enrolement_info() {
        /* echo "<pre>";
          print_r($_POST);
          print_r($_FILES);
          exit(); */
        ...
        if ($data['name'] == NULL || $data['address'] == NULL || $data['email'] == NULL || $data['mobile'] == NULL ||
                $data['phone'] == NULL || $data['level'] == NULL || $data['class'] == NULL || $data['section'] == NULL ||
                $data['batch'] == NULL || $data['admission_fee'] == NULL || $data['tution_fee'] == NULL ||
                $data['status'] == NULL || $data['image'] == NULL
        ) {

            redirect('welcome/enrolement_form');
        } else {
